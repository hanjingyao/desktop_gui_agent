...
1. click(x=<横坐标>, y=<纵坐标>) - 点击指定坐标。
   必须严格写成 click(x=数字, y=数字) 的形式，x= 和 y= 两个标签都不能省略，
   例如 click(x=274, y=433)，绝对不要写成 click(x=274, 433) 这种省略 y= 的形式。
...
- 打开桌面上的文件夹或图标：只用 double_click(x=整数, y=整数) 双击桌面上该图标的位置，
  这是唯一正确的方法。不要用 win 键、不要去开始菜单搜索、不要 type 文件夹名。
  双击后如果该文件夹窗口出现在屏幕上，就用 finish 结束。
- 不要在不同方法之间来回切换。选定一种方法后坚持用它，直到任务完成。
- type 只用于在输入框里输入内容，不能用来"打开"程序。
- 执行操作前，先观察当前屏幕处于什么状态，再决定下一步。
- 【铁律】只有在你已经真正执行过操作、并且当前屏幕上确实看到目标结果出现时，
  才能使用 finish。绝对禁止在没有执行任何操作的情况下就声称任务完成。
- 判断完成的标准必须基于"当前屏幕实际显示的内容"，而不是你的猜测或期望。
  例如要"打开此电脑"，只有当屏幕上真的出现了文件资源管理器窗口（能看到
  磁盘、文件夹列表）时，才算完成；如果屏幕上还是桌面，就说明还没完成，
  需要继续操作（如 double_click 此电脑图标的坐标）。
- 第一步通常应该是一个实际操作动作，而不是 finish。
- 如果你发现自己在重复同一个动作却没有进展，请重新观察屏幕，换一种思路，不要机械重复。
...
def parse_action(model_output: str) -> dict | None:
    ...
    # 匹配 "Action: 动作类型(参数...)"
# Action: 前缀可有可无，直接找「动作类型(参数)」
    match = re.search(r"(?:Action:\s*)?(\w+)\s*\((.*)\)", model_output, re.DOTALL)
    if not match:
    ...
    if action_type in ("click", "double_click") and "x" in params and "y" not in params:
        bare_numbers = re.findall(r'(?<![=\w])(\d+)(?![=\w])', params_str)
        if bare_numbers:
            params["y"] = int(bare_numbers[-1])
